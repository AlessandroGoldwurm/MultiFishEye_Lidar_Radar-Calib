...
void CameraCalibrator::set_input(
  ...
  std::cout << "start Finding corners" << std::endl;
  _boards_pts.clear();
  _imgs_pts.clear();
  int i = 0;
...
void CameraCalibrator::get_result(cv::Mat &camera_matrix, cv::Mat &k,
  ...
  double re_error =
      ...
      //                     k, rvecsMat, tvecsMat, cv::CV_CALIB_FIX_PRINCIPAL_POINT);
      cv::calibrateCamera(_boards_pts_3d, _imgs_pts, image_size, camera_matrix,
                          k, rvecsMat, tvecsMat, cv::CALIB_FIX_PRINCIPAL_POINT);
                          

  // // Affichage points 3d/2d du chessboard
  // for (size_t i = 0; i < _boards_pts_3d.size(); ++i) {
  //   std::cout << "Board " << i + 1 << " : " << std::endl;
  //   for (size_t j = 0; j < _boards_pts_3d[i].size(); ++j) {
  //     const cv::Point3f& point = _boards_pts_3d[i][j];
  //       std::cout << "  Point " << j + 1 << ": (" 
  ...
  //     const cv::Point2f& point = _imgs_pts[i][j];
  //     std::cout << "  Point " << j + 1 << ": (" 
  //     << point.x << ", " << point.y << ")" 
  //     << std::endl;
  //   }
  ...
  //   for (const auto &point : _imgs_pts[i]) {
  ...
  //   cv::destroyAllWindows();
  // }
  // for (size_t i = 0; i < _boards_pts.size(); ++i) {
  //   std::cout << "Image " << i + 1 << " : " << std::endl;
  //   for (size_t j = 0; j < _boards_pts[i].size(); ++j) {
  //     const cv::Point2f& point = _boards_pts[i][j];
  //     std::cout << "  Point " << j + 1 << ": (" 
  //     << point.x << ", " << point.y << ")" 
  //     << std::endl;
  //   }
  ...
  for (size_t i = 0; i < rvecsMat.size(); i++) {
    ...
    //  std::cout << "Points 3D des centres des cercles Lidar pour image " << i+1 << ":" << std::endl; 
    ...
    // std::cout << "Points 3D des centres de cercles (_boards_pts_cir), Simulés, pour l'image " << i+1 << ":" << std::endl;
    // for (const auto& point3D : _boards_pts_cir[i]) {
    //   std::cout << "(" << point3D.x << ", " << point3D.y << ", " << point3D.z << ")" << std::endl;
    //   }

    //Ici projection des centres des cercles de l'image i, dans le système image
    ...
    cv::projectPoints(_boards_pts_3d[i], rvecsMat[i], tvecsMat[i],
                  camera_matrix, k, test_img_board);

    cv::Mat img_copy = available_imgs[i].clone();
    std::cout << "Points projetés 2D (test_img_board) des coins, Simulés, pour l'image " << i+1 << ":" << std::endl;
  ...
  cv::Mat img = available_imgs[0].clone();
  cv::Mat undistort_img = img;
  cv::Point2f img_pt;
  lidar_projection(camera_intrinsic, initial_extrinsic,
                    lidar_point_pairs[0].lidar_3d_point[0], img_pt);
  cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);
  lidar_projection(camera_intrinsic, initial_extrinsic,
                       lidar_point_pairs[0].lidar_3d_point[1], img_pt);
  cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);
  lidar_projection(camera_intrinsic, initial_extrinsic,
                       lidar_point_pairs[0].lidar_3d_point[2], img_pt);
  cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);
  lidar_projection(camera_intrinsic, initial_extrinsic,
                  lidar_point_pairs[0].lidar_3d_point[3], img_pt);
  cv::circle(undistort_img, img_pt, 8, (0, 255, 0), 8);
