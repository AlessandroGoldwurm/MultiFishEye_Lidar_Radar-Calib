...
void NonlinearOptimizer::refine_sensor2camera_params(
  ...
  for (size_t i = 0; i < sensor2cam_point_pairs.size(); i++) {
    SensorCamPointPair _3d2d_pair = sensor2cam_point_pairs[i];
    Eigen::Vector3d sensor_3d_point(_3d2d_pair.sensor3d_point.x,
                                    _3d2d_pair.sensor3d_point.y,
                                    _3d2d_pair.sensor3d_point.z);
    Eigen::Vector2d camera_2d_point(_3d2d_pair.cam2d_point.x,
                                    _3d2d_pair.cam2d_point.y);

    SensorFisheyeReprojectionError *cost_function =
        new SensorFisheyeReprojectionError(v_camera_matrix, k, camera_2d_point, sensor_3d_point);

    problem.AddResidualBlock(
        new ceres::AutoDiffCostFunction<SensorFisheyeReprojectionError,
                                        2, 6>(cost_function),
        NULL, p_rt);//OPTIMIZE ONLY EXTRINSIC ?
  }
  // Configure the solver.
  ceres::Solver::Options options;
  options.linear_solver_type = ceres::DENSE_SCHUR;
  ...
  // DLOG(INFO) << "Final Brief Report:\n" << summary.BriefReport() <<
  // std::endl;
  this->formate_data(v_camera_matrix, k, rt, refined_params_);

}

void NonlinearOptimizer::refine_lidar2FEcamera_params(
    const LidarParams &params_,
    const std::vector<std::vector<cv::Point2f>> &imgs_pts_,
    const std::vector<std::vector<cv::Point2f>> &bords_pts_,
    const std::vector<LidarPointPair> lidar_point_pairs,
  ...
  // Ajouter les erreurs de reprojection pour les coins du damier
  for (int n = 0; n < params_.vec_rt.size(); ++n) {
    Eigen::MatrixXd x1(2, bords_pts_[n].size());
    Eigen::MatrixXd x2(2, imgs_pts_[n].size());
    ...
    double *p_rt = &packet_rt[n](0);
    for (int i = 0; i < x1.cols(); i++) {
      ReprojectionError_FE *cost_function =
        new ReprojectionError_FE(x2.col(i), x1.col(i));

      problem.AddResidualBlock(
        new ceres::AutoDiffCostFunction<ReprojectionError_FE,
                                        2, // num_residuals
                                        5, 2, 6>(cost_function),
        NULL, p_camera, p_k, p_rt);
    }
  ...
  LidarPointPair lidar_pair = lidar_point_pairs[i];
    for (size_t j = 0; j < 4; j++) {
      ...
      LidarFisheyeReprojectionError *cost_function =
          new LidarFisheyeReprojectionError(lidar_2d_point, lidar_3d_point);

      problem.AddResidualBlock(
          new ceres::AutoDiffCostFunction<LidarFisheyeReprojectionError,
                                          2, // num_residuals
                                          5, 2, 6>(cost_function),
          NULL, p_camera, p_k, p_rt);
      }
